...
@section('title', 'GEI - Aplicação')

@section('content_header')
    <h5 class="m-0">Aplicação</h5>
...
@section('content')
    ...
    <form method="POST"  action="{{ route('aplicacao.index') }}"  class="form-data-table align-center" id='form1'>
        @csrf
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-12">
                                @php
                                    $result = App\Http\Controllers\AplicacaoController::listaAplicacao();

                                    $heads1 = [
                                        ['label' => '', 'width' => '10'],
                                        ['label' => 'ID', 'width' => 10],
                                        ['label' => 'APLICAÇÃO', 'width' => 999],
                                    ];
                                ...
                                <x-adminlte-datatable id="table2" :heads="$heads1" head-theme="light" :config="$config1" striped hoverable bordered compact compressed  with-buttons>
                                    ...
                                    <tr>
                                        <td>
                                            <x-adminlte-button data-toggle="modal" data-target="#modalMin" class="btn btn-default text-primary mx-1 shadow" title="Edit"   icon="fas fa-pen"    data-id="{{ $row1['id'] }}"  data-aplicacao="{{ $row1['aplicacao'] }}" ></x-adminlte-button>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#modalMin').on('show.bs.modal', function(event) {
                ...
                var formAction = "{{ route('aplicacao.update', ':aplicacao') }}";
            ...
            $('#modalMinAdd').on('show.bs.modal', function(event) {
                ...
                modal.find('#aplicacao').val(aplicacao);
                // Atualizando a ação do formulário com o ID correto
                var formAction = "{{ route('aplicacao.store', ':aplicacao') }}";
                formAction = formAction.replace(':aplicacao', id);
        ...
        $('#form2').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: `${baseUrl}/aplicacao/${solicitacaoId}`,
                    data: formData,
                    processData: false,
                    contentType: false,
                    headers: {
...
// Metodo delete de aplicacao com confirmação através do sweetalert2
$(document).on('click', '.btn-delete', function() {
    const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
    const baseUrl = document.querySelector('meta[name="base-url"]').getAttribute('content');

    const aplicacaoId = this.getAttribute('data-id');

Swal.fire({
    title: 'Tem certeza que deseja deletar a Aplicação #' + aplicacaoId + '?',
    ...
    if (result.isConfirmed) {
        fetch( `${baseUrl}/aplicacao/${aplicacaoId}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': csrfToken
            }
        ...
        .then(response => {
            if (response.ok) {
                response.json().then(data => {
                    Swal.fire({
                        title:  'Aplicação Deletado com Sucesso!',
                ...
                response.json().then(data => {
                    Swal.fire({
                        title:  'Erro ao Deletar a Aplicação',
        ...
        .catch(error => {
            Swal.fire({
                title:  'Erro ao Deletar a Aplicacao',
                icon: 'warning',
                showConfirmButton: false,
                showCancelButton: false,
            });
...
<x-adminlte-modal id="modalMin" title="Aplicação" size="md" theme="teal" icon="fas fa-star" v-centered static-backdrop scrollable>
    <form method="POST"  action="{{ route('aplicacao.update', ['aplicacao' => 0]) }}"  class="form-data-table align-center" id='form2' name='form2' >
        @csrf
        @method('PUT')
        ...
        <div style="height:200px;">
            ...
            <input type='text' id="aplicacao" name="aplicacao" class='form-control'/>
        </div>
        <div class="card-body" name="footerSlot">
            <x-adminlte-button name="salvarAplicacao" id="salvaAplicacao" class="btn-flat" type="submit" label="Salvar" theme="success" icon="fas fa-lg fa-save"></x-adminlte-button>
            <x-adminlte-button theme="danger" label="Cancelar" data-dismiss="modal"/>
        </div>
    </form>
</x-adminlte-modal>

<x-adminlte-modal id="modalMinAdd" title="Aplicação" size="md" theme="teal" icon="fas fa-star" v-centered static-backdrop scrollable>
    <form method="POST"  action="{{ route('aplicacao.store') }}"  class="form-data-table align-center" id='form3' name='form3' >
        @csrf
        ...
        <div class="card-body" name="footerSlot">
            <x-adminlte-button name="addAplicacao" id="addAplicacao" class="btn-flat" type="submit" label="Salvar" theme="success" icon="fas fa-lg fa-save"></x-adminlte-button>
            <x-adminlte-button theme="danger" label="Cancelar" data-dismiss="modal"/>
        </div>
    </form>
</x-adminlte-modal>
